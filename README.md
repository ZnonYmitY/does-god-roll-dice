...
v1.0 React Demo 已实现：项目骨架、状态机、2D6 逻辑、文字库接入、动态低语、结果页、历史记录、分享图导出、正式字体，以及桌面背景、输入框、按钮、图标和 12 张骰面资产接入。UI、骰子和图标已使用透明裁切 WebP；24 张技能肖像已完成本地 WebP 接入。

掷骰阶段持续约 1.8 秒；结果页依次揭示骰点、结果等级、每条技能发言和最终判定。“换一批技能发言”同样重新执行逐条揭示，不会瞬间替换整组内容。

公开版本依照版权文档不携带原作技能肖像，图片请求失败时自动显示四属性 CSS 占位肖像。本地放置合法获得的 24 张肖像后无需修改业务代码即可显示。具体状态见 [ASSET_STATUS.md](ASSET_STATUS.md)。
...
该命令按 UI、骰子、图标和肖像分别处理，保留骰点等封闭深色细节。原始素材目录和原作肖像目录均受 `.gitignore` 约束，不随公开仓库发布。
